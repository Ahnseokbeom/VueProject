<template>
  <v-app class=app>
    <v-app-bar app color="rgb(145,216,131)" dark >
      ...
      <v-card>
        <v-card-actions>
          ...
          <v-btn
            ...
            @click="dialog = false"
          >
          <span style="width : 100%; height : 100%">X</span>
          </v-btn>
        </v-card-actions>
        <v-card-title>
          <span class="font">더 많은 액션/스포츠 게임</span>
        </v-card-title>
...
export default{ 
    ...
    setup(){
      ...
      axios.get("/api/as").then(({data})=>{
          state.items = data;
          console.log(data);
      }),
      axios.get("/api/detailAs").then(({data})=>{
        state.detail = data;
      })
      return {state}
    }
