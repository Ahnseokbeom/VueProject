<template>
  <v-app class=app>
    <v-app-bar app color="rgb(232, 208,177)" dark >
    ...
    <v-main>
      <div class="container">
      <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3" style="float : left">
...
export default{ 
    ...
    setup(){
      ...
      axios.get("/api/fps").then(({data})=>{
          state.items = data;
          console.log(data);
      }),
      axios.get("/api/detailFps").then(({data})=>{
        state.detail = data;
      })
      return {state}
    }
