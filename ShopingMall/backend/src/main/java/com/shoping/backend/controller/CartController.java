...
import com.shoping.backend.repository.CartRepository;
...
public class CartController {
	...
	public ResponseEntity getCartItems(@CookieValue(value="token", required = false)String token) {
		...
		List<Integer> itemIds = carts.stream().map(CartEntity::getItemId).collect(Collectors.toList());
		List<ItemEntity> items = itemRepository.findByIdIn(itemIds); 
	...
	public ResponseEntity pushCartItem(
		...
		int memberId = jwtService.getId(token);
		CartEntity cart = cartRepository.findByMemberIdAndItemId(memberId, itemId);
